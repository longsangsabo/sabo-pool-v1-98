...
export const useRealTimeTournamentState = () => {
  ...
  const checkRegistrationStatus = useCallback(async (tournamentId: string, retries = 2): Promise<boolean> => {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (error) {
          if (attempt === retries) {
            console.error('Final attempt failed for registration check:', error);
            return false;
          }
        ...
        const isRegistered = data && data.length > 0 && data[0].registration_status !== 'cancelled';
        console.log(`Registration check for ${tournamentId}:`, isRegistered, data);
        return isRegistered;
      } catch (error) {
        if (attempt === retries) {
  ...
  const loadRegistrationStatus = useCallback(async (tournamentIds: string[]) => {
    if (!user?.id || tournamentIds.length === 0) return;
    
    console.log('Loading registration status for tournaments:', tournamentIds);
  ...
  const setRegistrationStatus = useCallback((tournamentId: string, isRegistered: boolean) => {
    console.log(`Setting registration status for ${tournamentId}:`, isRegistered);
    setRegistrationState(prev => ({
      ...prev,
  ...
  const setLoadingState = useCallback((tournamentId: string, isLoading: boolean) => {
    setLoading(prev => ({
      ...prev,
  ...
  // Get loading status
  const isLoading = useCallback((tournamentId: string): boolean => {
    return loading[tournamentId] || false;
  }, [loading]);

  // Force refresh single tournament
  const refreshTournamentStatus = useCallback(async (tournamentId: string) => {
