...
const QuickClubCreator = () => {
  ...
  const districts = [
    ...
    'Quận 7', 'Quận 10', 'Quận Bình Thạnh', 'Quận Tân Bình',
    'Quận Phú Nhuận', 'Quận Gò Vấp', 'Quận Thủ Đức'
  ];

  const streets = [
    'Nguyễn Huệ', 'Lê Lợi', 'Hai Bà Trưng', 'Trần Hưng Đạo',
    'Nguyễn Thị Minh Khai', 'Võ Văn Tần', 'Cách Mạng Tháng 8',
    'Phan Xích Long', 'Hoàng Văn Thụ', 'Lý Tự Trọng'
  ];
  ...
  const generateClubData = () => {
    ...
    return {
      ...
      manager_phone: `09${Math.floor(Math.random() * 100000000).toString().padStart(8, '0')}`,
  ...
  const createClubs = async () => {
    ...
    try {
      ...
      // Insert club registrations
      const { data: insertedClubs, error: insertError } = await supabase
      ...
      // If auto-approve is enabled, create club profiles
      if (autoApprove && insertedClubs) {
        const clubProfiles = insertedClubs.map(club => ({
          user_id: club.user_id,
          club_name: club.club_name,
          address: club.address,
          phone: club.phone,
          operating_hours: {
            open: club.opening_time,
            close: club.closing_time,
            days: ['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun']
          },
          number_of_tables: club.table_count,
          verification_status: 'approved',
          verified_at: new Date().toISOString(),
          verified_by: null // Would be set to admin user in real scenario
        }));
        ...
        if (profileError) {
          console.warn('Failed to create club profiles:', profileError);
        }

        // Update user roles
        const userIds = insertedClubs.map(club => club.user_id);
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="autoApprove">Auto-approve clubs (create club profiles)</Label>
          </div>
        </div>

        <div className="p-4 bg-blue-50 rounded-lg">
          <h4 className="font-medium text-blue-900 mb-2">Generated Club Features:</h4>
        ...
        {createdClubs.length > 0 && (
          <div className="mt-6 p-4 bg-green-50 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="font-medium text-green-800">Clubs Created Successfully</h3>
            </div>
            <p className="text-sm text-green-700 mb-3">
              Created {createdClubs.length} test clubs 
              {autoApprove ? ' with approved status and club profiles' : ' pending approval'}.
            </p>
