...
const PlayerStatsComponent = () => {
  ...
  const [favoriteOpponents, setFavoriteOpponents] = useState<FavoriteOpponent[]>([]);
  ...
  const fetchPlayerStats = async () => {
    ...
    try {
      ...
      const opponentsWithProfiles = await Promise.all(
        (opponentsData || []).map(async (opponent) => {
          const { data: profile } = await supabase
            ...
            .eq('user_id', opponent.opponent_id)
            .single();
      ...
      setFavoriteOpponents(opponentsWithProfiles);
    } catch (error) {
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-700">{stats.matches_played}</p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${getWinRateColor(stats.win_rate)}`}>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${getStreakColor(stats.current_streak)}`}>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-700">{stats.longest_streak}</p>
              </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Badge className="bg-green-100 text-green-800">{stats.matches_won}</Badge>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Thua:</span>
              <Badge className="bg-red-100 text-red-800">{stats.matches_lost}</Badge>
            </div>
            <div className="flex justify-between items-center">
                ...
                {favoriteOpponents.map((opponent) => (
                  <div key={opponent.opponent_id} className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">{opponent.profiles.full_name}</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500">
                        {((opponent.wins / opponent.matches_count) * 100).toFixed(0)}%
                      </p>
                    </div>
                  </div>
                ))}
