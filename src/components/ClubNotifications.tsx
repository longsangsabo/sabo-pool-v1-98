...
import { Bell, Trophy, Users, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
...
const ClubNotifications = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const transformedNotifications: ClubNotification[] = (data || []).map(n => ({
        id: n.id,
        type: n.type,
        title: n.title,
        message: n.message,
            ...
            {filteredNotifications.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-sm mt-1">
                  {activeTab === 'unread' ? 'Tất cả thông báo đã được đọc' : 'Không có thông báo nào trong danh mục này'}
                </p>
              </div>
            ) : (
