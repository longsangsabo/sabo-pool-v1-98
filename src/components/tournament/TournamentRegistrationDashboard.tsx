...
import { 
  Users, Eye, Check, X, Clock, DollarSign, 
  Trophy, Calendar, MapPin, AlertCircle, CheckCircle 
} from 'lucide-react';
...
export const TournamentRegistrationDashboard: React.FC<TournamentRegistrationDashboardProps> = ({
  tournament,
  onClose
}) => {
  const { user } = useAuth();
                ...
                <div key={registration.id} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-medium">
                        {registration.user_profile.display_name || registration.user_profile.full_name}
                      </p>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <span>{new Date(registration.registration_date).toLocaleDateString('vi-VN')}</span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={getPaymentStatusColor(registration.payment_status)}>
                      ...
                      {registration.payment_status === 'pending' && 'Chờ thanh toán'}
                      {registration.payment_status === 'refunded' && 'Đã hoàn tiền'}
                    </Badge>
