...
export const ELOHistoryChart: React.FC<ELOHistoryChartProps> = ({
  ...
  const fetchHistoryData = async () => {
    ...
    try {
      const limit = timeRange === '7d' ? 50 : 
                   timeRange === '30d' ? 100 : 
                   timeRange === '90d' ? 200 : 500;
  ...
  const drawChart = () => {
    ...
    g.selectAll(".dot")
      ...
      .attr("r", 3)
      .attr("fill", d => 
        d.change > 0 ? "hsl(var(--success))" : 
        d.change < 0 ? "hsl(var(--destructive))" : "hsl(var(--muted))"
      )
      .attr("stroke", "white")
      .attr("stroke-width", 1);
  ...
  const getTimeRangeLabel = (range: string) => {
    switch (range) {
      ...
      case '90d': return '3 tháng';
      case '1y': return '1 năm';
      case 'all': return 'Tất cả';
      default: return '30 ngày';
    }
  };
