...
export const RealtimeRankingTracker: React.FC = () => {
  ...
  const handleMatchResultUpdate = (matchResult: any) => {
    ...
    const activity: RealtimeActivity = {
      ...
      timestamp: new Date(),
      eloChange: matchResult.winner_id === matchResult.player1_id 
  ...
  const handleRankingUpdate = (ranking: any) => {
    ...
    if (oldRank !== newRank) {
      const activity: RealtimeActivity = {
        id: `rank-${ranking.player_id}-${Date.now()}`,
        type: 'rank_change',
      ...
      if (notifications && ranking.player_id === user?.id) {
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              <Radio className={`h-5 w-5 ${isTracking ? 'text-green-500' : 'text-gray-500'}`} />
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Theo dõi các thay đổi ranking và hoạt động trận đấu trong thời gian thực.
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {activities.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  {isTracking ? 'Chờ hoạt động mới...' : 'Bật theo dõi để xem hoạt động'}
                </div>
              ) : (
                activities.map((activity) => (
                  <div key={activity.id} className="flex items-start gap-3 p-3 bg-muted/50 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm font-medium">{activity.playerName}</p>
                      ...
                      {activity.eloChange && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className={`text-xs font-medium ${
                            activity.eloChange > 0 ? 'text-green-600' : 'text-red-600'
                          }`}>
          ...
          <CardContent>
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {liveUpdates.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  {isTracking ? 'Chờ cập nhật ranking...' : 'Bật theo dõi để xem cập nhật'}
                </div>
              ) : (
                liveUpdates.map((update, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-sm">{update.playerName}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">Trận đấu hoàn thành</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
