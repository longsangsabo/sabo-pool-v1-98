...
const AvailabilityStatus = () => {
  const { myAvailability, updateAvailability, isUpdating } = usePlayerAvailability();
  
  const [status, setStatus] = useState(myAvailability?.status || 'unavailable');
  const [location, setLocation] = useState(myAvailability?.location || '');
  const [maxDistance, setMaxDistance] = useState(myAvailability?.max_distance_km || 5);

  const statusOptions = [
    { value: 'available_now', label: 'Rảnh bây giờ', color: 'bg-green-100 text-green-800' },
    { value: 'available_tonight', label: 'Rảnh tối nay', color: 'bg-blue-100 text-blue-800' },
    { value: 'available_weekend', label: 'Rảnh cuối tuần', color: 'bg-purple-100 text-purple-800' },
    { value: 'unavailable', label: 'Không rảnh', color: 'bg-gray-100 text-gray-800' },
  ];

  const handleUpdateStatus = () => {
    const availableUntil = status === 'available_now' 
      ...
      ? new Date(new Date().setDate(new Date().getDate() + (7 - new Date().getDay()))).toISOString() // End of weekend
      : null;
      ...
      <CardContent className="space-y-4">
          ...
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              {myAvailability.available_until && (
                <span className="text-xs text-gray-500 flex items-center gap-1">
                  ...
                  Đến {new Date(myAvailability.available_until).toLocaleTimeString('vi-VN', { 
                    hour: '2-digit', 
                    minute: '2-digit' 
                  })}
                </span>
            ...
            {myAvailability.location && (
              <div className="text-xs text-gray-500 flex items-center gap-1">
                ...
                {myAvailability.location} (bán kính {myAvailability.max_distance_km}km)
              </div>
            )}
          </div>
        )}
        ...
        <div className="space-y-3">
          ...
          {status !== 'unavailable' && (
            <>
              <div>
                <Label htmlFor="location" className="text-sm">Vị trí</Label>
                <Input
                  id="location"
                  value={location}
              ...
              <div>
                <Label htmlFor="distance" className="text-sm">Bán kính tìm kiếm</Label>
