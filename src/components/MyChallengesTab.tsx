...
const MyChallengesTab = () => {
  ...
  const fetchChallengesAndMatches = async () => {
    ...
    try {
      ...
      const upcomingWithProfiles = await Promise.all(
      ...
      const pastWithProfiles = await Promise.all(
        (past || []).map(async (match) => {
          const opponentId = match.player1_id === user.id ? match.player2_id : match.player1_id;
          const { data: profile } = await supabase
            .from('profiles')
            .select('full_name, avatar_url, current_rank')
      ...
      setIncomingChallenges(incomingWithProfiles as Challenge[]);
      setOutgoingChallenges(outgoingWithProfiles as Challenge[]);
        ...
        if (challenge) {
          const { error: matchError } = await supabase
            ...
            .insert({
              ...
              status: 'pending',
              played_at: challenge.scheduled_time
            });
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          {challenge.profiles?.current_rank} • {challenge.bet_points} điểm
                        </p>
                      </div>
                    </div>
