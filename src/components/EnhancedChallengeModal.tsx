...
const EnhancedChallengeModal = ({ isOpen, onClose, opponent }: EnhancedChallengeModalProps) => {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const challengeData = {
        ...
        stake_type: formData.stake_type,
        stake_amount: formData.stake_type === 'money' ? formData.stake_amount : 0,
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                ...
                {opponent.trust_percentage && (
                  <span className="ml-2">• {opponent.trust_percentage}% tin cậy</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2 mb-3">
              {[25, 50, 100, 200].map(points => (
                <Button
                  key={points}
                  variant={formData.bet_points === points ? 'default' : 'outline'}
                  onClick={() => setFormData(prev => ({ ...prev, bet_points: points }))}
                  className="h-12"
                >
                  {points}
                </Button>
          ...
          {/* Stake Type */}
          <div>
            <Label className="text-base font-medium mb-3 block">Loại cược</Label>
            <RadioGroup 
              value={formData.stake_type} 
              onValueChange={(value: 'friendly' | 'money' | 'drinks') => 
                setFormData(prev => ({ ...prev, stake_type: value, stake_amount: 0 }))
              }
            >
          ...
          {/* Date & Time */}
          <div>
            <Label htmlFor="scheduled_time" className="text-base font-medium mb-2 block">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <div className="flex">
                ...
                <div className="text-xs text-yellow-800">
                  <strong>Lưu ý:</strong> Đây là trận đấu có cược tiền thật. 
                  Hãy chắc chắn bạn và đối thủ đều đồng ý về số tiền cược trước khi bắt đầu.
                </div>
              </div>
            </div>
          )}
