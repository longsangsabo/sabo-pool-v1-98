...
const PostMatchRatingModal = ({ isOpen, onClose, matchId, opponentId, opponentName }: PostMatchRatingModalProps) => {
  const { user } = useAuth();
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          skill_assessment: selectedRating,
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <div className="flex">
              ...
              <div className="text-xs text-yellow-800">
                Đánh giá này sẽ ẩn danh với người được đánh giá nhưng được hệ thống ghi nhận để duy trì tính công bằng.
              </div>
            </div>
          </div>
