...
const ClubProfileForm = () => {
  ...
  const fetchClubProfile = async () => {
    try {
      ...
      if (data) {
        setClubProfile(data);
        const operatingHours = data.operating_hours as any;
        setFormData({
          club_name: data.club_name || '',
          address: data.address || '',
          phone: data.phone || '',
          number_of_tables: data.number_of_tables || 1,
          opening_time: operatingHours?.open || '',
          closing_time: operatingHours?.close || '',
          notes: ''
        });
      }
    } catch (error) {
