                ...
                <div key={penalty.id} className="border rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center text-gray-600">
                      ...
                      Từ: {new Date(penalty.start_date).toLocaleDateString('vi-VN')}
                      {penalty.end_date && (
                        <span className="ml-2">
                          đến: {new Date(penalty.end_date).toLocaleDateString('vi-VN')}
                        </span>
                      )}
                    </div>
                    ...
                    {penalty.appeal_reason && (
                      <div className="bg-blue-50 p-3 rounded">
                        ...
                        <p className="text-blue-800 mb-2">{penalty.appeal_reason}</p>
                        {penalty.appeal_date && (
                          <p className="text-xs text-blue-600">
                            Gửi ngày: {new Date(penalty.appeal_date).toLocaleDateString('vi-VN')}
                          </p>
                        )}
                        ...
                        {penalty.appeal_decision && (
                          <div className={`mt-2 p-2 rounded ${
                            ...
                            <strong>
                              Kết quả: {penalty.appeal_decision === 'approved' ? 'Chấp nhận' : 'Từ chối'}
                            </strong>
                            {penalty.appeal_decision_date && (
                              <p className="text-xs mt-1">
                                Ngày quyết định: {new Date(penalty.appeal_decision_date).toLocaleDateString('vi-VN')}
                              </p>
                            )}
                          </div>
                        )}
                  ...
                  {canAppeal(penalty) && (
                    <div className="mt-3 pt-3 border-t">
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => openAppealModal(penalty.id, penalty.reason)}
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Bạn có thể kháng cáo nếu cho rằng hình phạt không công bằng
                      </p>
                    </div>
                  )}
                </div>
