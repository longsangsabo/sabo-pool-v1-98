...
const RankTestModal = ({ request, onStartTest, onCompleteTest, processing }: RankTestModalProps) => {
  ...
  const getRankInfo = (rank: string) => {
    const rankInfo: Record<string, { name: string; description: string; requirements: string[] }> = {
      ...
      'J': { 
        name: 'Hạng J (Cơ bản)', 
        description: 'Có kỹ năng cơ bản, có thể chơi các ván đơn giản',
        requirements: ['Đánh chính xác 7/10 bi dễ', 'Biết tính toán đường bi', 'Có thể chơi liên tục 5-10 bi']
      },
      'I': { 
        name: 'Hạng I (Trung bình)', 
        description: 'Kỹ năng ổn định, có thể áp dụng chiến thuật',
        requirements: ['Đánh chính xác 8/10 bi trung bình', 'Biết các kỹ thuật xoáy', 'Có thể chơi liên tục 15-25 bi']
      }
    };
