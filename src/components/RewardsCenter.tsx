...
const RewardsCenter = () => {
  const { availableRewards, redemptions, redeemReward, isRedeeming } = useRewards();
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="mt-6">
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {redemptions.slice(0, 3).map((redemption) => {
                const reward = availableRewards.find(r => r.type === redemption.reward_type);
                return (
                  <div key={redemption.id} className="flex items-center gap-2 text-xs">
                    <span>{reward?.icon || '🎁'}</span>
                    <span className="flex-1">{reward?.name || redemption.reward_type}</span>
        ...
        <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-lg">
          <p>
            Kiếm điểm bằng cách check-in hàng ngày. Streak dài hơn = nhiều điểm hơn!
          </p>
        </div>
      </CardContent>
    </Card>
