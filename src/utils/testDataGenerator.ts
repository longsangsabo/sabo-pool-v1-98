...
export const getTestDataSummary = async () => {
  try {
    const [
      ...
      supabase.from('player_rankings').select('*', { count: 'exact' }).like('player_id', 'test-player-%'),
      ...
      supabase.from('spa_points_log').select('*', { count: 'exact' }).like('player_id', 'test-player-%')
    ]);
